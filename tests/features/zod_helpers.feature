Feature: Zod Helpers Behavior
    ...
    Scenario Outline: Failed Zod parsing maps each standard issue to a structured error

        Given a Zod schema with fields <fields>
        When I parse invalid data <data> with pureZodParse
        Then the result is a failure
        And the failure contains <count> errors
        And every error has type "processError"
        And every error has code "<code>"
        And every error data has a non-empty path
        And no error leaks the raw input or a stack trace

        Examples:
            | fields                          | data                                      | count | code         |
            | name:string,age:number          | {"name":"John","age":"thirty"}            | 1     | invalid_type |
            | name:string,age:number          | {"name":"John"}                           | 1     | invalid_type |
            | name:string,age:number          | {"name":123,"age":"abc"}                  | 2     | invalid_type |
    ...
    Scenario Outline: Failed Zod parsing with mixed errors
        ...
        When I parse data <data> with pureZodParse
        Then the result is a failure
        And the failure contains error codes "AGE_VALIDATION_FAILED" and "invalid_format"
    ...
    Scenario Outline: Converting a failed SafeParseResult maps each issue to a structured error

        Given a failed Zod SafeParseResult with <issueCount> generic issues
        When I convert it with convertZodParseResultToPureResult
        Then the result is a failure
        And the failure contains <issueCount> errors
        And every error has code "invalid_type"
